...
public class Parser {
			...
			throws IllegalArgumentException
	{
		...
		for(int i=0; i<3; i++)
		{
			...
			else if(this.lookAhead().getType().equals(Token.END_OF_INPUT_TYPE))
			{
				this.match(new String[]{"PrimFName", "UserFName", "Var", "SymbolLiteral", "[", "if", "let","find","with"});
			}
			else
			{
	...
	private void match(String[] expecteds)
			throws IllegalArgumentException
	{
		boolean result = false;
		Token next = this.lookAhead();
		for(String type : expecteds)
		{
			result |= next.getType().equals(type);
		}

		if(!result)
		{
			String expectedTypes = expecteds[0];
			for(int i=1; i<expecteds.length; )
			{
				expectedTypes += " or " + expecteds[i];
			}
			this.error(next.getSpelling(), expectedTypes);
		}
	}
